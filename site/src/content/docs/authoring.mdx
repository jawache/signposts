...
<Note>
**A rule is absolute — it blocks, or it doesn't exist.** There is no warn tier (`severity: warn` is a false green — it reads as enforced but stops nothing) and no per-instance escape hatch (a `# skip-me` marker or an `exempt:` name-list is rubber-stampable by the very agent the rule guards). If a check needs a bypass, or can't be scoped so that *every* hit is a true violation, it isn't a rule — make it a [sign](/docs/signs). Rules block; signs steer; nothing sits in between.
</Note>

## 1 · A rule from a core script — config only

Add a `type: rule` entry to a bundle's `signposts:` list, naming a [core script](/docs/rules) with `use:` and its config. No code.

```yaml
local:
  title: Local guardrails
  summary: Ad-hoc rules for this repo.
  signposts:
    - type: rule
      id: domain-has-test
      at: [write, commit]
...
Point `use:` at a script you write. It lives in *your* bundle's folder — `rules/<bundle>/` (core scripts live in `rules/core/`). It can be JavaScript or a shell script.
...
- **Per-file** (you only need the one changed file) → it runs **pre-emptively** on write and at commit. Default `at`, leave it off.
- **Whole-project** (you need the whole tree in a consistent state — an import graph, coverage) → it's a `kind: 'project'` [tool-gate](/docs/rules/tool-gate) or [depcruise](/docs/rules/depcruise) fence, `at: [commit]`. It can't run per keystroke, so it's commit-only.
...
## 3 · It belongs to a bundle

Your rule lives in a bundle's block in `signposts.yml` (`local` for your own ad-hoc rules; a name like `neon` if it's a coherent set you might share). The bundle is what [install and propagate](/docs/packs) move as a unit — its rules, its signs, its `settings`, and its `rules/<bundle>/` scripts, as one contiguous block.
