...
A rule fires at two [moments](/docs/wiring) by default (`at: [write, commit]`) — one definition, two places:

| Moment | Where | What it does |
| --- | --- | --- |
| `write` | Inside your agent, before a write | Pre-emptive block — reconstructs the would-be file, checks it, denies with the fix so the agent self-corrects. |
| `commit` | Your git hooks | The backstop gate over staged files — runs even on a plain `git commit`, with no AI involved. |

You only set `at:` when you want something other than both — a heavy whole-project check that can only run at `commit`, a `delete` guard, or an end-of-turn `turn` check. See [the moments](/docs/wiring), [the config file](/docs/signposts-yaml) and [the rules](/docs/rules).
