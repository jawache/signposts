...
`/signposts refresh` (agent-driven — see [the skill](/docs/skills)) does a **semantic three-way** against the pin: "upstream fixed this regex" merges cleanly under "you deliberately narrowed this rule's scope"; only upstream changed → take theirs; only you changed → keep + flag yours; *both* diverged → surface it. A conflicting script drops a `<file>.upstream` sidecar beside yours — never conflict markers in a live rule. A local improvement worth sharing gets offered back via [propagate](/docs/skills). That's what makes "install, then tune it locally" safe to combine with "stay current".
