...
This drops the whole core stack into your repo — the engine, the commit-gate hook (`.githooks/pre-commit`), the one config file (`signposts.yml`), a starter set of signs and rules — runs `npm install`, and arms the gate (`git config core.hooksPath .githooks`). Copying alone enforces nothing; the install is what makes it live.
